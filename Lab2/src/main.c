...
int main(int argc, char **argv){
    ...
    //TCP port = 21
    if(init(arguments.ip, 21, &socketfd_control) == -1){
    ...
    if(clientCommand(socketfd_control, command) == -1) return -1;
    if(passiveMode(socketfd_control, ip, &port)) return -1;

    printf("IP: %s\nPort Number: %d\n", ip, port);

    if((init(ip, port, &socketfd_data)) == -1){
    ...
    if(saveFile(socketfd_data, arguments.file_name) == -1) return -1;
